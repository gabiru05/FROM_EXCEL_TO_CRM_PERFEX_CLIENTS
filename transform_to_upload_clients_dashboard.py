...
def split_name(full_name):
    ...
    Maneja casos con múltiples nombres/apellidos y nombres vacíos.
    Prioriza nombres compuestos (ej., "María José")
    """
    if not full_name:
        return "", ""
    ...
    if len(parts) == 0:
        ...
        firstname = []
        lastname = []
        compound_names = ["María", "Ana", "Juan", "Luis", "José", "Carlos", "San", "Santa", "De", "Del", "La", "El", "Los"]
...
def process_excel(input_file, output_file):
    """Procesa el Excel de entrada y guarda los datos en el formato deseado."""

    try:
        # Intenta con el delimitador de tabulaciones y 'latin-1' primero
        df = pd.read_csv(input_file, sep='\t', encoding='latin-1')
    except (FileNotFoundError, pd.errors.ParserError, UnicodeDecodeError) as e1:
        try:
            # Si falla, intenta con Excel y 'utf-8'
             df = pd.read_excel(input_file, engine='openpyxl')
        except (FileNotFoundError, pd.errors.ParserError, UnicodeDecodeError) as e2:
             try:
                #Si todo lo demas falla intenta leer csv pero con utf-8
    ...
    for index, row in df.iterrows():
        ...
        #Un diccionario con los indices correctos segun el excel
        roles_mapping = {
    ...
    column_order = ['Firstname',	'Lastname',	'Email',	'Contact phonenumber',	'Position',
